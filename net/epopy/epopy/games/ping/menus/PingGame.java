...
public class PingGame extends AbstractGameMenu {
	...
	public void render() {
		...
		if (!gameOver && !paused && !pauseScreen) {
			if (timer != null) {
				boolean record = (long) timer.getTime() > Main.getPlayer().getPingStats().getRecord();
		...
		if (gameOver) {
			...
			if (!addStats) {
				...
				Main.getPlayer().getPingStats().addTemps((long) timer.getTime());
				// set best score
				if (record)
					Main.getPlayer().getPingStats().setRecord((long) timer.getTime());
					
				if (timer.getTime() * 1000 > pingStats.getObjectif()) {
