...
public class TankGame extends AbstractGameMenu {
	...
	private static final int maxRebonds = 3, tankSize = 25, maxBalles = 5;
	private static final double speedTank = 2;
	
	private static int timer;

	private boolean tir, win, stats, record, pauseScreen;
	private int cooldownRobot, robotBallesNbr, directionMax, recul, damaged, damage, tankAspectP, tankAspectR, print;
	private double speedRobot;
	private int[] distanceBlocks;
	private List<Location> tankPrintP, tankPrintR;
	private List<Ball> balles;
	private Location locPlayer, locRobot;
	...
	public void onEnable() {
		...
		gameStats = Main.getPlayer().getTankStats();
	...
	public void render() {
		...
		if (!pause.isFinish()) {
			...
			if (pause.getTimePauseTotal() == 5) {
				...
				if (gameStats.getRecord() > gameStats.getObjectif()) {
					...
					drawText(gameStats.getObjectif() + " fois le bot.", 710, 630, PosWidth.MILIEU, PosHeight.HAUT, 25, grey);
				}
		...
		if (gameOver) {
			if (!stats) {
				...
				if (damage > gameStats.getRecord()) {
					gameStats.setRecord(damage);
					record = true;
				}
	...
	class Ball {
		...
		private final boolean playerOwn, testBall;
		private final Location loc;
		
		private boolean isTestFinish, testResult;
