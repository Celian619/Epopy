...
public class TetrasGame extends AbstractGameMenu {
	...
	private static final int grilleSectionX = 40, grilleSectionY = 30;
	private static final double grilleWidth = defaultWidth / grilleSectionX, grilleHeight = defaultHeight / grilleSectionY;
	private static int timer;

	private boolean right, left, down, rotateL, rotateR, changeStopDown, stats, pauseScreen;
	private int score;
	private double fallSpeed = 0.075;
	private float rotation;
	private boolean[] isBlock;
	private float[] lastColor, color;
	private block movingBlock;

	@Override
	public void onEnable() {
		if (Main.getPlayer().hasSound() && !Audios.TETRAS.isRunning())
			Audios.TETRAS.start(true).setVolume(0.4f);
		song = Audios.TETRAS;
		score = 0;
		...
		gameStats = Main.getPlayer().getTetrasStats();
