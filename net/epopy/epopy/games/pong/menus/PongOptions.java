...
public class PongOptions extends AbstractGameMenu {
	...
	public static int KEY_DOWN = Integer.parseInt(Main.getPlayer().getConfig().getData("pong_control_bas", String.valueOf(Keyboard.KEY_DOWN)));
	public static int KEY_UP = Integer.parseInt(Main.getPlayer().getConfig().getData("pong_control_haut", String.valueOf(Keyboard.KEY_UP)));
	public static int MOUSE = Integer.parseInt(Main.getPlayer().getConfig().getData("pong_control_mouse", "2"));// 0 = mouse | 1 = touches |
	// 2 = touches & mouse
	...
	public void update() {
		...
		if (gauche.isClicked()) {
			...
			Main.getPlayer().getConfig().setValue("pong_control_mouse", String.valueOf(MOUSE));
		...
		if (controlBasClicked) {
			...
			for (int i = 0; i < 209; i++) {
				if (Keyboard.isKeyDown(i)) {
					KEY_DOWN = i;
					controlBasClicked = false;
					Main.getPlayer().getConfig().setValue("pong_control_bas", String.valueOf(KEY_DOWN));
					controlBas.setText(Keyboard.getKeyName(KEY_DOWN));
					break;
				}
			}
		...
		if (controlHautClicked) {
			...
			for (int i = 0; i < 209; i++) {
				if (Keyboard.isKeyDown(i)) {
					KEY_UP = i;
					controlHautClicked = false;
					Main.getPlayer().getConfig().setValue("pong_control_haut", String.valueOf(KEY_UP));
					controlHaut.setText(Keyboard.getKeyName(KEY_UP));
					break;
				}
			}
