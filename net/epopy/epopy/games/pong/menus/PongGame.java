...
public class PongGame extends AbstractGameMenu {
	...
	public void render() {
		...
		if (!gameOver && !paused && !pauseScreen) {
			if(timer != null) {
				boolean record = (long)timer.getTime() > Main.getPlayer().getPongStats().getRecord();
				ComponentsHelper.drawText(timeFormat.format(((long)timer.getTime() * 1000) - 3600000), defaultWidth / 2, 10, PositionWidth.MILIEU, PositionHeight.HAUT, 40, record ? colorTimer : new float[] { 1, 1, 1, 1 });
			}
		}
		...
		if (gameOver) {
			...
			boolean record = (timer.getTime()*1000) > pongStats.getRecord();
			renderEchap(false, timeString, record);
			if (!addStats) {
				Main.getPlayer().getPongStats().addPartie();
				Main.getPlayer().getPongStats().addTemps((long)timer.getTime());
				// set best score
				if (record)
					Main.getPlayer().getPongStats().setRecord((long)timer.getTime());

				if ((timer.getTime()*1000) > pongStats.getObjectif()) {
