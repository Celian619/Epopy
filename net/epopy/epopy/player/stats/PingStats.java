...
public class PingStats {
	...
	public PingStats(final Config config) {
		...
		if (Integer.parseInt(config.getData("configUpgrade", "0")) == 0) {
			config.setValue("ping_record", Integer.parseInt(config.getData("ping_record")) / 1000 + "");
			config.setValue("ping_temps", Integer.parseInt(config.getData("ping_temps")) / 1000 + "");
		}
