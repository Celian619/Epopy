...
public class WaitingRoom extends AbstractGameNetwork {
	...
	public void update() {
		...
		if(waitingRoom.getLeader().equals(NetworkPlayer.getNetworkPlayer().getName())) {
			...
			if(jouer.isClicked() && !waitingRoom.getWaitingRoomStatus().equals(WaitingRoomStatus.MATCH_FOUND)) {
				Packets.sendPacket(NetworkPlayer.getNetworkPlayer().getNetworkPlayerHandlersWaitingRoom(), new PacketPlayerWaitingRoom(NetworkPlayer.getNetworkPlayer(), NetworkPlayer.getNetworkPlayer().getName(), waitingRoom.getWaitingRoomStatus().equals(WaitingRoomStatus.WAITING) ? PacketWaitingRoomType.START: PacketWaitingRoomType.CANCEL_START));	
	 ...
	 * 
	 * @param button du joueur
	 */
	private ButtonGui showProfil = new ButtonGui("Voir le profil", new float[]{1, 1, 1, 1}, 22, false);
	private ButtonGui sendFriendRequest = new ButtonGui("Ajouter en ami", new float[]{1, 1, 1, 1}, 22, false);
	private ButtonGui sendMessage = new ButtonGui("Message privée", new float[]{1, 1, 1, 1}, 22, false);
	private ButtonGui kickPlayer = new ButtonGui("Exclure", new float[]{1, 0, 0, 1}, 22, false);

	private void renderInfosPlayer(ButtonGui button, String playerName, boolean leader) {
		int x = button.xx + 80;
		int y = button.yy + 55;
